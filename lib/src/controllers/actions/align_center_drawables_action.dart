...
class AlignCenterDrawablesAction extends ControllerAction<void, void> {
  AlignCenterDrawablesAction();
  ...
  void perform$(PainterController controller) {
    ...
    double minDx = fist.position.dx - fist.getSize().width / 2;
    ...
    currentSelectedDrawables.forEach((item) => {
          ...
          if (item.position.dx - item.getSize().width / 2 < minDx)
            {minDx = item.position.dx - item.getSize().width / 2}
        });
